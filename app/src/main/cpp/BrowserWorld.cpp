...
void
BrowserWorld::StartWidgetMove(int32_t aHandle) {
  ASSERT_ON_RENDER_THREAD();
  VRB_LOG("makelele1");
  WidgetPtr widget = m.GetWidget(aHandle);
  if (!widget) {
  ...
  m.movingWidget->StartMoving(widget, controllerIndex, initialPoint);
}
...
(JNIEnv*, jobject, jint aHandle) {
  crow::BrowserWorld::Instance().StartWidgetMove(aHandle);
}
